...
class AuthorizationChecker{
     ...
     */
    requireLogin(redirectMsg=null){
        return (req,res,next)=>{
            const msg=(!!redirectMsg)?redirectMsg:this.msg.LoginFail;
     ...
     */
    requireRole(ROLE_STR, redirectMsg=null){

        return (req, res, next) => {
            const msg=(!!redirectMsg)?redirectMsg:this.msg.RoleFail;
     ...
     */
    requireAnyRole(ROLES_ARRAY, redirectMsg=null) {
        return (req, res, next) => {
            const msg=(!!redirectMsg)?redirectMsg:this.msg.RoleFail;
            return this.rolesAccessor(req)
                .then(roles=>{
     ...
     */
    requireAllRoles(ROLES_ARRAY, redirectMsg=null) {
        return (req, res, next) => {
            const msg=(!!redirectMsg)?redirectMsg:this.msg.RoleFail;
            return this.rolesAccessor(req)
                .then(roles=>{
     ...
     */
    requireTrue(fn=(req)=>true,redirectMsg=null){
        return (req, res, next) => {
            const msg=(!!redirectMsg)?redirectMsg:this.msg.RoleFail;
